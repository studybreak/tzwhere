...
var init = function(tzWorldFile, callback) {
  ...
  if(currentTzWorld !== tzWorldFile) {
    ...
    fs.readFile(constructedShortcutFilePath, {encoding: 'utf-8'}, function (err, cache) {
      if (err) {
        fs.readFile(tzWorldFile, {encoding: 'utf-8'}, function (err, data) {
          if (err) return callback(err);
          constructShortcuts(null, data);
          callback();
        });
      }
      else {
        constructShortcuts(cache, null);
        callback();
      }
    });
...
var constructShortcuts = function (cache, data) {
  // Construct once
  if ((timezoneNamesToPolygons === null) || (timezoneLongitudeShortcuts === null)) {
    ...
    if (cache) {
      console.log('loading tz info from cache');
      ...
      fs.writeFile(constructedShortcutFilePath, JSON.stringify({
      ...
      console.log(Date.now() - now + 'ms to construct shortcut table and write to file');
    }

    console.log(Date.now() - now + 'ms to load shortcut table');
  }
};
